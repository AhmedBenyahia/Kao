...
void verifydataSign() {
    ...
    g_signal_connect(G_OBJECT(PINOK), "clicked", G_CALLBACK(Hide), (gpointer) SamePin);
    g_signal_connect(G_OBJECT(PINCLOSE), "clicked", G_CALLBACK(Hide),
                     (gpointer) SamePin);           ////////////////LINK FONCTIONS ///////////////
    ...
    if (strcmp(gtk_entry_get_text(GTK_ENTRY(FName)), "\0") == 0 ||
    ...
    else if (strcmp(gtk_entry_get_text(GTK_ENTRY(Password)), gtk_entry_get_text(GTK_ENTRY(RPassword))) !=
             0)            //if pin are similar condition
        gtk_widget_show(SamePin);
        ...
        if (stat("src/Data.txt", &st) == 0) {
                ...
                0)                                                                               //check file size if 0 fill it with data
            {
                ...
                fclose(fp);
                fp = fopen("src/Data.txt",
                           "r");                                                               //if its not empty read it
                while (fgets(buffer, 80, fp)) {
                    if (i % 2 != 0) {
                        i++;
                        continue;
                ...
                for (int k = 0; k < j; k++) {
                    strncpy(word, message[k], strlen(message[k]) - 1);
                    if (strcmp(gtk_entry_get_text(GTK_ENTRY(Email)), word) ==
                        0) {                            //compara the entred data with all users in file
                        test = true;
                    }
                }
                if (test) {
                    gtk_widget_show(
                            UserUsed);                                                           //if there is a duplication alert
                    test = false;
                } else {
                    fclose(fp);                                                                         //else close file and open it as write privilege
                    fp = fopen("src/Data.txt", "ab");
...
void verifydataLog() {
    ...
    builder = gtk_builder_new_from_file("glade/window_main.glade");
    AccountError = GTK_WIDGET(gtk_builder_get_object(builder,
                                                     "AccountError"));                          ///////////////////////////////////////////
    AccountEmpty = GTK_WIDGET(gtk_builder_get_object(builder, "AccountEmpty"));
    Profile = GTK_WIDGET(gtk_builder_get_object(builder, "Profile"));
    Wrongpassword = GTK_WIDGET(gtk_builder_get_object(builder, "Wrongpassword"));
    ...
    fermer = GTK_WIDGET(gtk_builder_get_object(builder, "fermer"));
    OKK = GTK_WIDGET(
            gtk_builder_get_object(builder, "OKK"));                                                   // & windows
    CLOSEE = GTK_WIDGET(gtk_builder_get_object(builder, "CLOSEE"));
    Cancel = GTK_WIDGET(gtk_builder_get_object(builder, "Cancel"));
    Okay = GTK_WIDGET(gtk_builder_get_object(builder, "Okay"));
    Yes = GTK_WIDGET(gtk_builder_get_object(builder, "YES"));
    No = GTK_WIDGET(gtk_builder_get_object(builder,
                                           "NO"));                                    /////////////////////////////////////////////
    LNameLabel = GTK_LABEL(gtk_builder_get_object(builder, "LNameLabel"));
    FNameLabel = GTK_LABEL(gtk_builder_get_object(builder, "FNameLabel"));
    EmailLabel = GTK_LABEL(gtk_builder_get_object(builder, "EmailLabel"));
    ...
    g_signal_connect(G_OBJECT(Cancel), "clicked", G_CALLBACK(Hide),
                     (gpointer) Wrongpassword);                           //link with functions
    g_signal_connect(G_OBJECT(Okay), "clicked", G_CALLBACK(Hide),
                     (gpointer) Wrongpassword);                             //link with functions

    g_signal_connect(G_OBJECT(LogOut), "clicked", G_CALLBACK(Hide),
                     (gpointer) Profile);                                //link with functions
    g_signal_connect(G_OBJECT(LogOut), "released", G_CALLBACK(main), (gpointer) Profile);

    g_signal_connect(G_OBJECT(Changer), "released", G_CALLBACK(openfile), (gpointer) Profile);


    g_signal_connect(G_OBJECT(can), "clicked", G_CALLBACK(Hide),
                     (gpointer) NoAccount);                                  //link with functions
    g_signal_connect(G_OBJECT(okey), "clicked", G_CALLBACK(Hide),
                     (gpointer) NoAccount);                                 //link with functions

    g_signal_connect(G_OBJECT(ok), "clicked", G_CALLBACK(Hide),
                     (gpointer) AccountError);                                //link with functions
    g_signal_connect(G_OBJECT(fermer), "clicked", G_CALLBACK(Hide),
                     (gpointer) AccountError);                            //link with functions

    g_signal_connect(G_OBJECT(OKK), "clicked", G_CALLBACK(Hide),
                     (gpointer) AccountEmpty);                               //link with functions
    g_signal_connect(G_OBJECT(CLOSEE), "clicked", G_CALLBACK(Hide),
                     (gpointer) AccountEmpty);                            //link with functions
    ...
    if (strcmp(gtk_entry_get_text(GTK_ENTRY(LEmail)), "\0") != 0 &&
        ...
        fp = fopen("src/Data.txt",
                   "r");                                                                                   //open a file and only read
        while (fgets(buffer, 80,
                     fp)) {                                                                                 //while there is a line enter to the loop
            if (i % 2 == 0) {
                strcpy(message[j],
...
>>>>>>> 6ad3970a5d9f71fd7b9de9b2defa35a6e44d22b6
        ...
        for (int k = 0; k < j; k++) {
            ...
            strncpy(word, message[k], strlen(message[k]) -
                                      1);                                                          //this is used to delete the '\n' because when you get data from a file \n is the symbol of an end of line in buffer
            strncpy(word1, messagepass[k], strlen(messagepass[k]) -
                                           1);                                                 //this is used to delete the '\n' because when you get data from a file \n is the symbol of an end of line in buffer

            if (strcmp(gtk_entry_get_text(GTK_ENTRY(LEmail)), word) == 0 &&
                strcmp(gtk_entry_get_text(GTK_ENTRY(LPassword)), word1) ==
                0) {    //if data corresponds to info in the file
            ...
            if (strcmp(gtk_entry_get_text(GTK_ENTRY(LEmail)), word) == 0 &&
                strcmp(gtk_entry_get_text(GTK_ENTRY(LPassword)), word1) != 0) {
...
void popup_exit() {
    ...
    g_signal_connect(G_OBJECT(YES), "clicked", G_CALLBACK(gtk_main_quit),
                     (gpointer) Exit);                   //LINK POP UP EXIT BUTTONS TO ITS FUNCTIONS
